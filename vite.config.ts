  ...
  build: {
    ...
    lib: {
      ...
      fileName: (format: string) => {
        ...
        return `jsonforms-vue-primevue.${format}.js`;
      },
    },
    rollupOptions: {
      external: [
      ...
      output: {
        globals: (name: string) => {
          if (name === 'vue') {
            return 'Vue';
          } else if (name.startsWith('primevue')) {
            // primevue/button -> PrimeVue.Button, primevue -> PrimeVue
            if (name === 'primevue') {
              return 'PrimeVue';
            }
            const componentName = name.split('/')[1];
            return 'PrimeVue.' + componentName.charAt(0).toUpperCase() + componentName.slice(1);
          } else if (name === 'primeicons') {
            return 'Primeicons';
        ...
        assetFileNames: 'jsonforms-vue-primevue.[ext]',
      },
    },
